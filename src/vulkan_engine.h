...
struct SCamera {
  ...
  SCamera(glm::vec3 pos = glm::vec3(0.0f, 0.0f, 0.0f),
      ...
      : position(pos), world_up(up), yaw(initial_yaw), pitch(initial_pitch),
        movement_speed(2.5f), wheel_speed(0.01f), mouse_sensitivity(0.1f),
        zoom(45.0f),
...
#define FRAME_INDEX_TO_UNIFORM_BUFFER_ID(frame_index) (frame_index + 4)
  ...
  std::unordered_map<std::string, std::vector<vra::ResourceId>>
      mesh_vertex_resource_ids_;
  std::unordered_map<std::string, std::vector<vra::ResourceId>>
      mesh_index_resource_ids_;
  ...
  std::map<vra::BatchId, vra::VraDataBatcher::VraBatchHandle>
      vertex_index_staging_batch_handle_;
  std::map<vra::BatchId, vra::VraDataBatcher::VraBatchHandle>
      uniform_batch_handle_;
  ...
  std::map<vra::BatchId, vra::VraDataBatcher::VraBatchHandle>
      test_local_host_batch_handle_;
  std::map<vra::BatchId, vra::VraDataBatcher::VraBatchHandle>
      test_uniform_batch_handle_;
