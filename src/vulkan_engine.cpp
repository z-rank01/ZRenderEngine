...
VulkanEngine &VulkanEngine::GetInstance() { return *instance; }
...
void VulkanEngine::GetVertexIndexData(
    std::vector<gltf::PerDrawCallData> per_draw_call_data,
...
void VulkanEngine::GetMeshList(
    const std::vector<gltf::PerMeshData> &mesh_list) {
...
VulkanEngine::~VulkanEngine() {
  ...
  if (descriptor_set_layout_ != VK_NULL_HANDLE) {
    vkDestroyDescriptorSetLayout(vkb_device_.device, descriptor_set_layout_,
                                 nullptr);
  ...
  if (uniform_buffer_ != VK_NULL_HANDLE) {
    vmaDestroyBuffer(vma_allocator_, uniform_buffer_,
...
void VulkanEngine::InitializeSDL() {
  ...
  if (!vkWindowHelper_->GetWindowBuilder()
           ...
           .SetWindowSize(engine_config_.window_config.width,
...
void VulkanEngine::InitializeCamera() {
  ...
  camera_.up = glm::vec3(
      0.0f, 1.0f, 0.0f); // up direction is +y (because Y-axis is up in Vulkan)
...
void VulkanEngine::ProcessInput(SDL_Event &event) {
  ...
  if (event.type == SDL_EVENT_KEY_DOWN) {
    ...
    if (event.key.key == SDLK_F) {
      ...
      Logger::LogInfo(camera_.focus_constraint_enabled_
                          ? "Focus constraint enabled"
  ...
  if (event.type == SDL_EVENT_MOUSE_MOTION) {
    ...
    float y_offset =
        last_y_ - y_pos; // Invert y-offset as screen y increases downwards
    last_x_ = x_pos;
    last_y_ = y_pos;
    ...
    if (free_look_mode_) {
      ...
      if (camera_.has_focus_point && camera_.focus_constraint_enabled_) {
        ...
        float distance_factor =
            glm::clamp(current_distance / camera_.focus_distance,
                       camera_.min_focus_distance / camera_.focus_distance,
                       camera_.max_focus_distance / camera_.focus_distance);
        sensitivity_scale =
            distance_factor; // Slower when closer to focus point
      }

      // Apply sensitivity and scale
      float actual_x_offset =
          x_offset * camera_.mouse_sensitivity * sensitivity_scale;
      float actual_y_offset =
          y_offset * camera_.mouse_sensitivity * sensitivity_scale;
    ...
    if (camera_pan_mode_) {
      ...
      float actual_pan_speed_multiplier =
          camera_.focus_constraint_enabled_
              ? pan_speed_multiplier / distance_scale
              : pan_speed_multiplier;

      // calculate the target movement amount
      float target_x_offset =
          x_offset * camera_.movement_speed * actual_pan_speed_multiplier;
      float target_y_offset =
          y_offset * camera_.movement_speed * actual_pan_speed_multiplier;
...
void VulkanEngine::ProcessKeyboardInput(float delta_time) {
  ...
  if (free_look_mode_) {
    ...
    if (keyboard_state[SDL_SCANCODE_Q]) {
      movement +=
          camera_.up *
          current_velocity; // Using camera's up vector for local up/down
    }
...
// Main render loop
void VulkanEngine::Draw() { DrawFrame(); }
...
void VulkanEngine::GenerateFrameStructs() {
  ...
  for (int i = 0; i < engine_config_.frame_count; ++i) {
    ...
    output_frames_[i].command_buffer_id =
        "graphic_command_buffer_" + std::to_string(i);
    output_frames_[i].image_available_semaphore_id =
        "image_available_semaphore_" + std::to_string(i);
    output_frames_[i].render_finished_semaphore_id =
        "render_finished_semaphore_" + std::to_string(i);
...
bool VulkanEngine::CreateInstance() {
  ...
  auto instance_chain =
      create_context() | set_application_name("My Vulkan App") |
      set_engine_name("My Engine") | set_application_version(1, 3, 0) |
      add_validation_layers({"VK_LAYER_KHRONOS_validation"}) |
      add_extensions({"VK_EXT_debug_utils"}) | validate_context() |
      create_vk_instance();
...
bool VulkanEngine::CreatePhysicalDevice() {
  ...
  auto phys_ret =
      selector.set_surface(vkWindowHelper_->GetSurface())
  ...
  if (!phys_ret) {
    std::cout << "Failed to select Vulkan Physical Device. Error: "
...
bool VulkanEngine::CreateLogicalDevice() {
  ...
  if (!dev_ret) {
    std::cout << "Failed to create Vulkan device. Error: "
...
bool VulkanEngine::CreateSwapChain() {
  ...
  auto swap_ret =
      swapchain_builder
          ...
          .set_desired_extent(engine_config_.window_config.width,
                              engine_config_.window_config.height)
          .set_image_usage_flags(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT)
          .build();

  if (!swap_ret) {
    std::cout << "Failed to create Vulkan swapchain. Error: "
              << swap_ret.error().message() << std::endl;
    return false;
  }
  ...
  swapchain_config_.device_extensions_.push_back(
...
bool VulkanEngine::CreateCommandPool() {
  ...
  return vkCommandBufferHelper_->CreateCommandPool(
      vkb_device_.device,
...
bool VulkanEngine::CreateVertexInputBuffers() {
  ...
  vra::VraDataDesc vertex_data_desc{vra::VraDataMemoryPattern::GPU_Only,
  ...
  index_buffer_create_info.usage =
      VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
  index_buffer_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  vra::VraDataDesc index_data_desc{vra::VraDataMemoryPattern::GPU_Only,
                                   vra::VraDataUpdateRate::RarelyOrNever,
  ...
  vra::VraDataDesc staging_data_desc{vra::VraDataMemoryPattern::CPU_GPU,
                                     vra::VraDataUpdateRate::RarelyOrNever,
                                     staging_buffer_create_info};

  // collect and group
  vra_data_batcher_->Collect(vertex_data_desc, vertex_raw_data,
                             vertex_data_id_);
  vra_data_batcher_->Collect(index_data_desc, index_raw_data, index_data_id_);
  vra_data_batcher_->Collect(staging_data_desc, vertex_raw_data,
                             staging_vertex_data_id_);
  vra_data_batcher_->Collect(staging_data_desc, index_raw_data,
                             staging_index_data_id_);
  vertex_index_staging_batch_handle_ = vra_data_batcher_->Batch();

  // get buffer create info
  const auto &local_buffer_create_info =
      vertex_index_staging_batch_handle_[vra::VraBuiltInBatchIds::GPU_Only]
          .data_desc.GetBufferCreateInfo();

  VmaAllocationCreateInfo alloc_info = {};
  alloc_info.usage = VMA_MEMORY_USAGE_AUTO;
  alloc_info.flags = vra_data_batcher_->GetSuggestVmaMemoryFlags(
      vra::VraDataMemoryPattern::GPU_Only,
      vra::VraDataUpdateRate::RarelyOrNever);
  if (!Logger::LogWithVkResult(vmaCreateBuffer(vma_allocator_,
                                               &local_buffer_create_info,
                                               &alloc_info, &local_buffer_,
  ...
  const auto &host_buffer_create_info =
      vertex_index_staging_batch_handle_
          [vra::VraBuiltInBatchIds::CPU_GPU_Rarely]
              .data_desc.GetBufferCreateInfo();
  const auto &consolidated_data = vertex_index_staging_batch_handle_
                                      [vra::VraBuiltInBatchIds::CPU_GPU_Rarely]
                                          .consolidated_data;

  VmaAllocationCreateInfo staging_alloc_info = {};
  staging_alloc_info.usage = VMA_MEMORY_USAGE_AUTO;
  staging_alloc_info.flags = vra_data_batcher_->GetSuggestVmaMemoryFlags(
      vra::VraDataMemoryPattern::CPU_GPU,
      vra::VraDataUpdateRate::RarelyOrNever);
  if (!Logger::LogWithVkResult(
          vmaCreateBuffer(vma_allocator_, &host_buffer_create_info,
                          &staging_alloc_info, &staging_buffer_,
  ...
  vmaInvalidateAllocation(vma_allocator_, staging_buffer_allocation_, 0,
                          VK_WHOLE_SIZE);
  ...
  vmaFlushAllocation(vma_allocator_, staging_buffer_allocation_, 0,
                     VK_WHOLE_SIZE);
...
bool VulkanEngine::CreateUniformBuffers() {
  for (int i = 0; i < engine_config_.frame_count; ++i) {
    ...
    vra::VraDataDesc data_desc{vra::VraDataMemoryPattern::CPU_GPU,
                               vra::VraDataUpdateRate::Frequent,
                               buffer_create_info};
    vra::VraRawData raw_data{&current_mvp_matrix, sizeof(SMvpMatrix)};
    uniform_buffer_id_.push_back(0);
    vra_data_batcher_->Collect(data_desc, std::move(raw_data),
                               uniform_buffer_id_.back());
  }
  ...
  const auto &uniform_buffer_create_info =
      uniform_batch_handle_[vra::VraBuiltInBatchIds::CPU_GPU_Frequently]
          .data_desc.GetBufferCreateInfo();

  VmaAllocationCreateInfo allocation_create_info = {};
  allocation_create_info.usage = VMA_MEMORY_USAGE_AUTO;
  allocation_create_info.flags = vra_data_batcher_->GetSuggestVmaMemoryFlags(
      vra::VraDataMemoryPattern::CPU_GPU, vra::VraDataUpdateRate::Frequent);
  allocation_create_info.requiredFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                         VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
  if (!Logger::LogWithVkResult(
          vmaCreateBuffer(vma_allocator_, &uniform_buffer_create_info,
                          &allocation_create_info, &uniform_buffer_,
                          &uniform_buffer_allocation_,
...
bool VulkanEngine::CreateAndWriteDescriptorRelatives() {
  ...
  VkDescriptorType dynamic_uniform_buffer_type =
  ...
  if (!Logger::LogWithVkResult(vkCreateDescriptorPool(vkb_device_.device,
                                                      &pool_info, nullptr,
                                                      &descriptor_pool_),
  ...
  if (!Logger::LogWithVkResult(
          vkCreateDescriptorSetLayout(vkb_device_.device, &layout_info, nullptr,
  ...
  if (!Logger::LogWithVkResult(vkAllocateDescriptorSets(vkb_device_.device,
                                                        &alloc_info,
                                                        &descriptor_set_),
...
bool VulkanEngine::CreateVmaVraObjects() {
  // vra and vma members
  vra_data_batcher_ = std::make_unique<vra::VraDataBatcher>(
      vkb_physical_device_.physical_device);
...
VkFormat VulkanEngine::FindSupportedDepthFormat() {
  ...
  for (VkFormat format : candidates) {
    ...
    vkGetPhysicalDeviceFormatProperties(vkb_physical_device_.physical_device,
                                        format, &props);
...
bool VulkanEngine::CreateDepthResources() {
  ...
  // 创建图像
  if (vkCreateImage(vkb_device_.device, &image_info, nullptr, &depth_image_) !=
      VK_SUCCESS) {
  ...
  vkGetImageMemoryRequirements(vkb_device_.device, depth_image_,
                               &mem_requirements);
  ...
  vkGetPhysicalDeviceMemoryProperties(vkb_physical_device_.physical_device,
                                      &mem_properties);
  ...
  // 分配内存
  if (vkAllocateMemory(vkb_device_.device, &alloc_info, nullptr,
                       &depth_memory_) != VK_SUCCESS) {
    Logger::LogError("Failed to allocate depth image memory");
    return false;
  }

  // 绑定内存到图像
  if (vkBindImageMemory(vkb_device_.device, depth_image_, depth_memory_, 0) !=
      VK_SUCCESS) {
  ...
  if (vkCreateImageView(vkb_device_.device, &view_info, nullptr,
...
bool VulkanEngine::CreateFrameBuffer() {
  ...
  SVulkanFrameBufferConfig framebuffer_config(
      swapchain_config->target_swap_extent_, swapchain_image_views,
      depth_image_view_);

  vkFrameBufferHelper_ = std::make_unique<VulkanFrameBufferHelper>(
      vkb_device_.device, framebuffer_config);

  return vkFrameBufferHelper_->CreateFrameBuffer(
      vkRenderpassHelper_->GetRenderpass());
}
...
bool VulkanEngine::CreatePipeline() {
  ...
  std::string shader_path =
      engine_config_.general_config.working_directory + "src\\shader\\";
  ...
  configs.push_back(
      {EShaderType::kFragmentShader, fragment_shader_path.c_str()});

  for (const auto &config : configs) {
    std::vector<uint32_t> shader_code;
    if (!vkShaderHelper_->ReadShaderCode(config.shader_path, shader_code)) {
      Logger::LogError("Failed to read shader code from " +
                       std::string(config.shader_path));
      return false;
    }

    if (!vkShaderHelper_->CreateShaderModule(vkb_device_.device, shader_code,
                                             config.shader_type)) {
      Logger::LogError("Failed to create shader module for " +
                       std::string(config.shader_path));
      return false;
    }
  }
  ...
  renderpass_config.depth_format =
      VK_FORMAT_D32_SFLOAT; // TODO: Make configurable
  renderpass_config.sample_count =
      VK_SAMPLE_COUNT_1_BIT; // TODO: Make configurable
  vkRenderpassHelper_ =
      std::make_unique<VulkanRenderpassHelper>(renderpass_config);
  ...
  pipeline_config.shader_module_map = {
      {EShaderType::kVertexShader,
       vkShaderHelper_->GetShaderModule(EShaderType::kVertexShader)},
      {EShaderType::kFragmentShader,
       vkShaderHelper_->GetShaderModule(EShaderType::kFragmentShader)}};
  ...
  pipeline_config.vertex_input_binding_description =
      test_vertex_input_binding_description_;
  pipeline_config.vertex_input_attribute_descriptions =
      test_vertex_input_attributes_;
...
bool VulkanEngine::AllocatePerFrameCommandBuffer() {
  for (int i = 0; i < engine_config_.frame_count; ++i) {
    if (!vkCommandBufferHelper_->AllocateCommandBuffer(
            ...
            output_frames_[i].command_buffer_id)) {
      Logger::LogError("Failed to allocate command buffer for frame " +
                       std::to_string(i));
      return false;
    }
  }
...
bool VulkanEngine::CreateSynchronizationObjects() {
  vkSynchronizationHelper_ =
      std::make_unique<VulkanSynchronizationHelper>(vkb_device_.device);
  // create synchronization objects
  for (int i = 0; i < engine_config_.frame_count; ++i) {
    if (!vkSynchronizationHelper_->CreateVkSemaphore(
            output_frames_[i].image_available_semaphore_id))
      return false;
    if (!vkSynchronizationHelper_->CreateVkSemaphore(
            output_frames_[i].render_finished_semaphore_id))
...
void VulkanEngine::DrawFrame() {
  ...
  auto current_image_available_semaphore_id =
      output_frames_[frame_index_].image_available_semaphore_id;
  auto current_render_finished_semaphore_id =
      output_frames_[frame_index_].render_finished_semaphore_id;
  auto current_command_buffer_id =
      output_frames_[frame_index_].command_buffer_id;
  ...
  // get semaphores
  auto *image_available_semaphore = vkSynchronizationHelper_->GetSemaphore(
      current_image_available_semaphore_id);
  auto *render_finished_semaphore = vkSynchronizationHelper_->GetSemaphore(
      current_render_finished_semaphore_id);
  ...
  if (acquire_result == VK_ERROR_OUT_OF_DATE_KHR ||
    ...
    Logger::LogWithVkResult(acquire_result, "Failed to acquire next image",
  ...
  command_buffer_submit_info.commandBuffer =
      vkCommandBufferHelper_->GetCommandBuffer(current_command_buffer_id);
  ...
  if (!Logger::LogWithVkResult(
          vkQueueSubmit2(
              vkb_device_.get_queue(vkb::QueueType::graphics).value(), 1,
  ...
  VkResult present_result = vkQueuePresentKHR(
      vkb_device_.get_queue(vkb::QueueType::graphics).value(), &present_info);
  if (present_result == VK_ERROR_OUT_OF_DATE_KHR ||
      present_result == VK_SUBOPTIMAL_KHR) {
    resize_request_ = true;
    return;
  } else if (present_result != VK_SUCCESS) {
    Logger::LogWithVkResult(present_result, "Failed to present image",
...
bool VulkanEngine::RecordCommand(uint32_t image_index,
                                 std::string command_buffer_id) {
  // 更新当前帧的 Uniform Buffer
  UpdateUniformBuffer(image_index);

  // begin command recording
  if (!vkCommandBufferHelper_->BeginCommandBufferRecording(
          command_buffer_id, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT))
    return false;

  // collect needed objects
  auto command_buffer =
      vkCommandBufferHelper_->GetCommandBuffer(command_buffer_id);
  auto swapchain_config = &swapchain_config_;
  ...
  vkCmdCopyBuffer(command_buffer, test_staging_buffer_, test_local_buffer_, 1,
                  &buffer_copy_info);
  ...
  renderpass_info.framebuffer =
      (*vkFrameBufferHelper_->GetFramebuffers())[image_index];
  ...
  vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    vkPipelineHelper_->GetPipeline());

  // bind descriptor set
  auto offset =
      uniform_batch_handle_[vra::VraBuiltInBatchIds::CPU_GPU_Frequently]
          .offsets[uniform_buffer_id_[frame_index_]];
  uint32_t dynamic_offset = static_cast<uint32_t>(offset);
  vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          vkPipelineHelper_->GetPipelineLayout(), 0, 1,
                          &descriptor_set_, 1, &dynamic_offset);
  ...
  viewport.y = 0.0f;
  viewport.width =
      static_cast<float>(swapchain_config->target_swap_extent_.width);
  viewport.height =
      static_cast<float>(swapchain_config->target_swap_extent_.height);
  ...
  vkCmdBindVertexBuffers(
      command_buffer, 0, 1, &test_local_buffer_,
      &test_local_host_batch_handle_[vra::VraBuiltInBatchIds::GPU_Only]
           .offsets[test_vertex_buffer_id_]);
  vkCmdBindIndexBuffer(
      command_buffer, test_local_buffer_,
      test_local_host_batch_handle_[vra::VraBuiltInBatchIds::GPU_Only]
          .offsets[test_index_buffer_id_],
...
void VulkanEngine::UpdateUniformBuffer(uint32_t current_frame_index) {
  ...
  // update the view matrix
  mvp_matrices_[current_frame_index].view =
      glm::lookAt(camera_.position,                 // camera position
  ...
  vmaMapMemory(vma_allocator_, uniform_buffer_allocation_,
               &uniform_buffer_mapped_data_);
  ...
  uint8_t *data_location =
      static_cast<uint8_t *>(uniform_buffer_mapped_data_) + offset;

  // copy the data to the mapped memory
  memcpy(data_location, &mvp_matrices_[current_frame_index],
         sizeof(SMvpMatrix));
...
void VulkanEngine::FocusOnObject(const glm::vec3 &object_position,
...
void VulkanEngine::CreateMeshListBuffer() {
  ...
  for (const auto &mesh : mesh_list_) {
    ...
    for (const auto &primitive : mesh.primitives) {
      ...
      vra::VraDataDesc vertex_buffer_desc{vra::VraDataMemoryPattern::GPU_Only,
                                          vra::VraDataUpdateRate::RarelyOrNever,
                                          vertex_buffer_create_info};
      ...
      vra::VraDataDesc index_buffer_desc{vra::VraDataMemoryPattern::GPU_Only,
                                         vra::VraDataUpdateRate::RarelyOrNever,
                                         index_buffer_create_info};
      ...
      vra::VraDataDesc staging_vertex_buffer_desc{
          vra::VraDataMemoryPattern::CPU_GPU,
          vra::VraDataUpdateRate::RarelyOrNever, staging_buffer_create_info};

      staging_buffer_create_info.size =
          index_buffer_data.size_; // 索引数据的暂存缓冲区
      ...
      if (!vra_data_batcher_->Collect(vertex_buffer_desc, vertex_buffer_data,
                                      vertex_id)) {
        Logger::LogError("Failed to collect vertex buffer data for mesh: " +
                         mesh.name);
        continue;
      }
      if (!vra_data_batcher_->Collect(index_buffer_desc, index_buffer_data,
                                      index_id)) {
        Logger::LogError("Failed to collect index buffer data for mesh: " +
                         mesh.name);
        continue;
      }
      if (!vra_data_batcher_->Collect(staging_vertex_buffer_desc,
                                      vertex_buffer_data, staging_vertex_id)) {
        Logger::LogError(
            "Failed to collect staging vertex buffer data for mesh: " +
            mesh.name);
        continue;
      }
      if (!vra_data_batcher_->Collect(staging_index_buffer_desc,
                                      index_buffer_data, staging_index_id)) {
        Logger::LogError(
            "Failed to collect staging index buffer data for mesh: " +
            mesh.name);
        continue;
      }
  ...
  VmaAllocationCreateInfo allocation_create_info{};
  allocation_create_info.usage = VMA_MEMORY_USAGE_AUTO;
  vmaCreateBuffer(vma_allocator_, &test_local_buffer_create_info,
                  &allocation_create_info, &test_local_buffer_,
                  &test_local_buffer_allocation_,
                  &test_local_buffer_allocation_info_);

  // 创建暂存缓冲区
  auto test_host_buffer_create_info =
      test_local_host_batch_handle_[vra::VraBuiltInBatchIds::CPU_GPU_Rarely]
          .data_desc.GetBufferCreateInfo();
  ...
  staging_allocation_create_info.flags =
      vra_data_batcher_->GetSuggestVmaMemoryFlags(
          vra::VraDataMemoryPattern::CPU_GPU,
          vra::VraDataUpdateRate::RarelyOrNever);
  vmaCreateBuffer(vma_allocator_, &test_host_buffer_create_info,
                  &staging_allocation_create_info, &test_staging_buffer_,
                  &test_staging_buffer_allocation_,
                  &test_staging_buffer_allocation_info_);

  // 复制数据到暂存缓冲区
  auto consolidate_data =
      test_local_host_batch_handle_[vra::VraBuiltInBatchIds::CPU_GPU_Rarely]
          .consolidated_data;
  void *data;
  vmaInvalidateAllocation(vma_allocator_, test_staging_buffer_allocation_, 0,
                          VK_WHOLE_SIZE);
  ...
  vmaFlushAllocation(vma_allocator_, test_staging_buffer_allocation_, 0,
                     VK_WHOLE_SIZE);

  // 设置顶点输入绑定描述
  test_vertex_input_binding_description_.binding = 0;
  test_vertex_input_binding_description_.stride = sizeof(gltf::Vertex);
  test_vertex_input_binding_description_.inputRate =
  ...
  // uv1
  test_vertex_input_attributes_.push_back(
      VkVertexInputAttributeDescription{.location = 5,
                                        .binding = 0,
                                        .format = VK_FORMAT_R32G32_SFLOAT,
...
void VulkanEngine::CreateDrawCallListBuffer() {
  vra::VraRawData vertex_buffer_data{.pData_ = vertices_.data(),
                                     .size_ = sizeof(gltf::Vertex) *
                                              vertices_.size()};
  vra::VraRawData index_buffer_data{
      .pData_ = indices_.data(), .size_ = sizeof(uint32_t) * indices_.size()};
  ...
  if (!vra_data_batcher_->Collect(vertex_buffer_desc, vertex_buffer_data,
                                  test_vertex_buffer_id_)) {
    Logger::LogError("Failed to collect vertex buffer data");
    return;
  }
  if (!vra_data_batcher_->Collect(index_buffer_desc, index_buffer_data,
                                  test_index_buffer_id_)) {
    Logger::LogError("Failed to collect index buffer data");
    return;
  }
  if (!vra_data_batcher_->Collect(staging_vertex_buffer_desc,
                                  vertex_buffer_data,
                                  test_staging_vertex_buffer_id_)) {
    Logger::LogError("Failed to collect staging vertex buffer data");
    return;
  }
  if (!vra_data_batcher_->Collect(staging_index_buffer_desc, index_buffer_data,
                                  test_staging_index_buffer_id_)) {
...
void VulkanEngine::ReleaseTestData() {
  // release vma
  vmaDestroyBuffer(vma_allocator_, test_local_buffer_,
                   test_local_buffer_allocation_);
  vmaDestroyBuffer(vma_allocator_, test_staging_buffer_,
                   test_staging_buffer_allocation_);
